...
void Memcard_CreateImageGroup(void) {
    {
        ...
        __asm__("" : : : "memory");
        imageData = (MemcardImage *)(D_80193254 + D_80193274);
        node->kind = kind;
    ...
    Memcard_CreateImageNode(3);
    Memcard_CreateFixedImageNode();
    Memcard_CreateImageNode(5)->field14 = D_80192F98;
    {
        register s32 kind;
        MemcardImageNode *node = D_801D136C;
        register MemcardImageNode *tail;
        ...
        node->parameter = 0; node->kind = 0; node->field30 = 0;
        __asm__("" : : : "memory");
        imageData = (MemcardImage *)(D_80193254 + D_80193270);
        node->kind = kind;
        node->image = imageData;
        {
            register MemcardImage *image asm("$3") = node->image;
