...
int Camera_TrackRelativeOffset(int **args) {
    ...
    if ((U16_AT(init_state, 8) & 0x20) == 0) {
        turn_speed = *args[2];
        target_x = current_x + *args[0];
        target_z = current_z + *args[1];
        U16_AT(init_state, 8) |= 0x20;
        S32_AT(init_state, 0x20) = 0;
        S32_AT(init_state, 0x14) = turn_speed;
        S32_AT(init_state, 0x1C) = target_z;
        S32_AT(init_state, 0x18) = target_x;
    } else {
        turn_speed = S32_AT(init_state, 0x14);
        target_x = S32_AT(init_state, 0x18);
        target_z = S32_AT(init_state, 0x1C);
    }
