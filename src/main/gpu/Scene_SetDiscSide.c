...
int Scene_SetDiscSide(s8 side) {
    ...
    if (side == -1) {
        side = (s8)SCENE_STORY_DAY_PENDING;
        SCENE_STORY_DAY_CURRENT = SCENE_STORY_DAY_PENDING;
        SCENE_STORY_DAY_FLAGS |= 3;
    }

    if (((D_8009D1A0 & 2) != 0) || ((*(u32 *)GAME_STATE & 2) != 0)) {
        SCENE_STORY_DAY_FLAGS |= 2;
        D_8009D2E8 &= ~2U;
    }

    flags = SCENE_STORY_DAY_FLAGS;
    if ((flags & 4) != 0) {
        SCENE_STORY_DAY_FLAGS = (flags | 3) & ~4;
    }

    if ((u32)(side - 1) < 8U) {
        if (side != (s8)SCENE_STORY_DAY_PENDING) {
            SCENE_STORY_DAY_PENDING = side;
            SCENE_STORY_DAY_CURRENT = side;
            SCENE_STORY_DAY_FLAGS |= 1;
        }
    }
