...
the generated bytes. A trial pin on the table pointer was removable and is not
retained. No CPU instruction assembly or empty barrier is used.

`scene_e08::func_80193C68` matches all 776 retail bytes in C. Six short-lived
constant pins retain the register allocation across its effect-palette writes.
Four empty constraints preserve the scale store, early constant setup, and
final sound-call argument timing. Trial pins on the sound pointer, scale, pan,
...
the selected-slot and sequence stores in order. A separate declaration of the
path buffer retains the second address load before `Save_BuildCardFile`. Each
constraint was removed individually and broke the match; an unnecessary alias
of the port-state array was removed. The surrounding `tu_031908` remains an
assembly subsegment until its other functions match.

`Render_AnimationFrame` matches all 424 retail bytes with typed actor and
frame-data fields. Two `$4` register pins cover separate branches; one empty
barrier keeps the frame-counter reload in the long-animation branch. Two
address aliases keep the VRAM-counter load and update separate. Removing any
of those constraints changes the generated bytes. An otherwise unused 12-byte
local array retains the original stack frame. `Save_DrawSlotMetadata` remains
in the adjacent assembly subsegment.

`MenuInput_DispatchQueuedEvents` matches all 472 retail bytes in C. Its one
