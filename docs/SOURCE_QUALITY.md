...
### Render_TransformMorphVertices

The 644-byte function composes each part matrix with the view matrix using
scratchpad 0x1F800000, then projects visible parts in triples. It advances
both screen/depth output pointers by three words before storing each
triple's result, as retail does. Both loop counts are reloaded after the
GTE output writes; guarded do/while loops preserve zero-count behavior.
Matrix translation packs the low halves of X/Y and loads Z separately.
The shared output/work buffers D_800B1638 and D_800A6360 now have one u32
array declaration; the previous byte view in Render_DrawObjectAlt uses an
explicit cast, preserving its indexing and generated bytes.

Matching debt: 24 register pins, 11 empty barriers, four pointer/integer
casts, raw-offset access and an artificial eight-byte frame reservation.
Individual GTE transfer macros and authorized hazard nops are used. Native
stock GCC 2.7.2, compiler flags and maspsx remain unmodified.
Relocatable-object Levenshtein score is 15 solely because three target
