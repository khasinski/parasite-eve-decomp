...
| `0x64CD4..0x64F70` | `psyq/libapi/intr_dma` | Public Psy-Q `etc/intr_dma.c` orders `startIntrDMA`, `trapIntrDMA`, `setIntrDMA`, and `memclrIntrDMA` exactly as this range. The intervening 384-byte asm was previously misnamed as a renderer; its DMA interrupt dispatch, bus-error handling, handler table, and diagnostic strings identify it as `trapIntrDMA`. The compiled helpers preserve all 284 `.text` bytes exactly, and `make check` is retail-identical. |
| `0x64BB4..0x64CD4` | `psyq/libapi/intr_vsync` | Public Psy-Q `etc/intr_vb.c` orders `startIntrVSync`, `trapIntrVSync`, `setIntrVSync`, and `memclr` exactly as this range. The 108-byte intervening asm increments the VSync count and dispatches its eight callbacks, identifying it as `trapIntrVSync`; the compiled helpers preserve all 168 `.text` bytes exactly and use the shared registrar prototype. |
