#!/usr/bin/env python3
"""Compare ordinary-C MulMatrix0 CPU-side behavior against retail with scripted GTE results."""
import hashlib
import itertools
...
def run(data,results,layout,candidate):
    ...
    pc=entries['MulMatrix0'] if candidate else int(symbols['MulMatrix0'],16)
    code_size=next(len(payload) for address,payload in sections if address==0x150000) if candidate else 0x10C
    ...
    expected[out:out+20]=struct.pack('<9h',*flat)+struct.pack('<h',results[2][2]>>16)
...
for data,results in cases:
    for layout in layouts:
        assert run(data,results,layout,False)==run(data,results,layout,True)
        count+=1
print(f'PASS {count} cases: rotation control writes, three input columns, command/read order, scripted IR results, nine overlap layouts, full arena preservation, padding, return and canaries; GTE arithmetic/timing not emulated')
