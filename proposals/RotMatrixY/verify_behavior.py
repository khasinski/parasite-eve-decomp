#!/usr/bin/env python3
"""Compare ordinary-C RotMatrixY against retail, modeling external APIs."""
import hashlib
import itertools
...
with tempfile.TemporaryDirectory(prefix='rotate-y-') as work:
...
def expected(angle, data):
    ...
    rows = struct.unpack_from('<9h', data)
    for column in range(3):
        x,y = rows[column],rows[column+6]
        for row,word in ((0,cosine*x+sine*y),(2,cosine*y-sine*x)):
...
def run(angle, data, candidate):
    ...
    pc=entries['RotMatrixY'] if candidate else int(symbols['RotMatrixY'],16)
