#!/usr/bin/env python3
"""Compare ordinary-C RotMatrix against retail, modeling external APIs."""
import hashlib
import itertools
...
def expected(angles,data):
    ...
    a=q(cz,-sy);b=q(sz,-sy)
    rows=(q(cz,cy),q(-sz,cy),sy,
          q(sz,cx)-q(a,sx),q(cz,cx)+q(b,sx),q(-cy,sx),
          q(sz,sx)+q(a,cx),q(cz,sx)-q(b,cx),q(cy,cx))
...
def run(angles,data,alias,candidate):
    ...
    pc=entries['RotMatrix'] if candidate else int(symbols['RotMatrix'],16)
