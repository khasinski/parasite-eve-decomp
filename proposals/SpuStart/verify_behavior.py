#!/usr/bin/env python3
"""Compare ordinary-C SpuStart against retail, modeling external APIs."""
import hashlib
import itertools
...
with tempfile.TemporaryDirectory(prefix='spu-start-') as work:
...
def run(called, callback, event, opened_event, candidate):
    ...
    names = ('EnterCriticalSection', '_SpuDataCallback', 'OpenEvent', 'EnableEvent', 'ExitCriticalSection')
    ...
    pc = entries['SpuStart'] if candidate else int(symbols['SpuStart'], 16)
    for step in range(10):
        cpu.emu_start(pc, 0x801E0000, count=10000)
        if not trapped: break
        name = api[trapped.pop()]
        arg = cpu.reg_read(UC_MIPS_REG_A0)
        if name == 'EnterCriticalSection': assert read(0x9B3EC) == 1
        if name == '_SpuDataCallback':
            assert arg == int(symbols['_spu_FiDMA'], 16)
        if name == 'OpenEvent':
            args = tuple(cpu.reg_read(r) for r in (UC_MIPS_REG_A0, UC_MIPS_REG_A1, UC_MIPS_REG_A2, UC_MIPS_REG_A3))
            assert args == (0xF0000009, 0x20, 0x2000, 0)
            trace.append((name, *args))
        elif name == 'EnableEvent':
            assert arg == opened_event and read(0x9B384) == opened_event
            trace.append((name, arg))
        elif name == '_SpuDataCallback': trace.append((name, arg))
        else: trace.append((name,))
        cpu.reg_write(UC_MIPS_REG_V0, opened_event if name == 'OpenEvent' else 0xDEADBEEF)
        pc = cpu.reg_read(UC_MIPS_REG_RA)
    assert cpu.reg_read(UC_MIPS_REG_PC) == 0x801E0000
    assert cpu.reg_read(UC_MIPS_REG_SP) == 0x801F0000
    assert [t[0] for t in trace if t[0] in names] == (list(names) if called == 0 else [])
    return trace, (read(0x9B3EC), read(0x9B434), read(0x9B438), read(0x9B384))

for count, case in enumerate(itertools.product((0, 1, 2, 0xFFFFFFFF), (0, 0x80123400), (0, 0xF1000001), (0, 0xF1000002, 0xFFFFFFFF)), 1):
    assert run(*case, False) == run(*case, True), case
print(f'PASS {count} cases: startup guard, called store before critical entry, DMA callback identity, event arguments/return propagation, global accesses, API order and stack')
