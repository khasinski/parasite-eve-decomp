#!/usr/bin/env python3
"""Compare ordinary-C RotMatrixYXZ against retail and an independent integer model."""
import hashlib
import itertools
...
def expected(angles,data):
    ...
    a=q(sy,sx);b=q(cy,sx)
    rows=(q(cy,cz)+q(a,sz),-q(cy,sz)+q(a,cz),q(sy,cx),
          q(sz,cx),q(cz,cx),-sx,
          -q(sy,cz)+q(b,sz),q(sy,sz)+q(b,cz),q(cy,cx))
...
def run(angles,data,alias,candidate):
    ...
    pc=entries['RotMatrixYXZ'] if candidate else int(symbols['RotMatrixYXZ'],16)
