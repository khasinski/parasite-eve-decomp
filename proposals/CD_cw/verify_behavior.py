...
real_sync = '--real-sync' in sys.argv[2:]
...
with tempfile.TemporaryDirectory(prefix='cd-cw-') as work:
    ...
    script.write_text('\n'.join(definitions) + '\nSECTIONS { .text 0x8007B558 : { *(.text) } .rodata 0x80140000 : { *(.rodata*) } /DISCARD/ : { *(.reginfo) *(.MIPS.abiflags) *(.pdr) *(.comment) *(.gnu.attributes) } }')
...
def run(command, null_parameter, null_result, mode, scenario, callbacks, debug, candidate):
    ...
    def access(u, kind, address, size, value, user):
        address &= 0x1FFFFFFF
        if not 0x130000 <= address <= 0x130002:
            return
        assert size == 1
        if kind == UC_MEM_WRITE:
            if real_sync and address == 0x130001:
                state.update(phase=1, vsync=0, interrupt=0)
    ...
    state = {'vsync': 0, 'interrupt': 0, 'phase': 0}
    def call(u, address):
        name = api[address]
        ...
        if name == 'printf':
            argc = 4 if args[0] == 0x80011B28 else 2
        recorded = tuple(args[:argc])
        if name == 'printf' and argc == 4:
    ...
    writes = [item for item in trace if item[0] == 'write']
    ...
    memory = tuple(bytes(cpu.mem_read(address, size)) for address, size in ((0x9AFD0, 6), (0x9B294, 3), (0xA3460, 16), (0xA3478, 12), (0x140100, 8)))
...
print(f"PASS {count} cases ({'executed CD_sync' if real_sync else 'modeled CD_sync'}): parameter validation, state, FIFO, callbacks, timeout paths and results")
