#!/usr/bin/env python3
"""Compare CD_ready C with retail MIPS under finite modeled CD FIFO traces."""
import hashlib
import itertools
...
with tempfile.TemporaryDirectory(prefix='cd-ready-') as work:
...
def run(mode, initial, null_result, scenario, callbacks, candidate):
    ...
    cpu.mem_write(0x9B294, bytes([2, initial[0], initial[1]]))
    ...
    def call(u, address):
        ...
        if name == 'VSync':
            ...
            state['interrupt'] += 1
            if value & 4:
                u.mem_write(0x9B295, b'\x05' if scenario == 'error' else b'\x01')
    ...
    assert result in (0, 1, 4, 5, 0xFFFFFFFF)
    ...
    if result in (1, 4, 5):
        if result == 4:
            assert cpu.mem_read(0x9B296, 1)[0] == 0
            source = 0xA3470
        else:
            assert bytes(cpu.mem_read(0x9B295, 2)) == b'\0\0'
            source = 0xA3468
        if not null_result:
            assert cpu.mem_read(0x140100, 8) == cpu.mem_read(source, 8)
    else:
        assert cpu.mem_read(0x140100, 8) == b'\xA5' * 8
    memory = tuple(bytes(cpu.mem_read(address, size)) for address, size in ((0x9B294, 3), (0xA3460, 24), (0xA3478, 12), (0x140100, 8)))
    return result, trace, memory

cases = itertools.product((0, 1, -1), tuple(itertools.product((0, 1, 5), (0, 4))), (False, True), ('poll', 'dispatch', 'error', 'deadline', 'counter', 'deadline_boundary', 'counter_boundary'), range(4))
for count, case in enumerate(cases, 1):
    assert run(*case, False) == run(*case, True), case
print(f'PASS {count} cases: end/ready priority and consumption, responses, callbacks, MMIO and timeouts')
