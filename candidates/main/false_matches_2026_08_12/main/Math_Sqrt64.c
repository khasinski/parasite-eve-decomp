...
double Math_Sqrt64(double left, double right) {
    ...
    lhsMantissa = Math_Shift64(lhsMantissa, 1, 21);
    rhsMantissa = Math_Shift64(rhsMantissa, 1, 21);
    ...
    partial = Math_Shift64(partial, 1, 21);
    ...
    if ((int)product.hi < 0) {
        ...
        product = Math_Shift64(product, 1, 11);
    } else {
        rounding.lo = 0x200;
        product = Math_Add64(product, rounding);
        if (product.hi & 0x80000000) {
            ...
            product = Math_Shift64(product, 1, 10);
        }
    }
