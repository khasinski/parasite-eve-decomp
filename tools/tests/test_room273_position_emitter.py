...
SOURCE = ROOT / 'src/overlays/room_m273/RoomEffect_PositionEmitter.c'
...
typedef char layout[sizeof(Record)==8 && __alignof__(Record)==2 &&
    ...
    (unsigned long)&((Record *)0)->unknown==6 &&
...
int main(void) {
    ...
    for(stop=0;stop<3;++stop) for(emit=0;emit<3;++emit)
        for(fail=0;fail<2;++fail) {
            output.x=11; output.y=22; output.z=33; output.unknown=0xA55A;
            ...
            calls=!stop && emit;
            assert(allocs==calls);
            assert(output.x==(calls && !fail ?65535:11));
            assert(output.y==(calls && !fail ?32768:22));
            assert(output.z==(calls && !fail ?123:33));
            assert(output.unknown==0xA55A);
