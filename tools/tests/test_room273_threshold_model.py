...
SOURCE = ROOT / 'src/overlays/room_m273/RoomEffect_ThresholdModel.c'
...
void func_800C6EF8(void *p) {
    assert(stage++==4 && p==assets[0]); D_8019AE8C=assets[1]; effect.parameter12=65535;
}
...
void Gte_ScaleMatrix(Matrix *m,Vector *v) {
    ...
    m->m[0][0]=4321; effect.x=11; effect.y=22; effect.z=-333; D_8019AE8C=assets[2];
}
void func_800C71E4(void *p,Matrix *m) {
    assert(stage++==10 && p==assets[2] && m->m[0][0]==4321);
    assert(m->t[0]==11 && m->t[1]==22 && m->t[2]==-333); D_8019AE8C=assets[3];
}
...
int main(void) {
    ...
    for(wantedKind=3;wantedKind<=4;++wantedKind) for(wantedVariant=0;wantedVariant<2;++wantedVariant) {
        memset(&effect,0xA5,sizeof(effect)); stage=0; D_8019AE8C=assets[0];
