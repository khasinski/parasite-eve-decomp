...
SOURCE = ROOT / 'src/overlays/room_m273/RoomEffect_SampledLayerEmitter.c'


class SampledLayer(unittest.TestCase):
    def test_target_layout(self):
        source = SOURCE.read_text() + r'''
typedef char layout[sizeof(Effect)==12 && sizeof(unsigned long)==4 &&
    (unsigned long)&((Effect *)0)->x==0 &&
    (unsigned long)&((Effect *)0)->y==2 &&
    (unsigned long)&((Effect *)0)->z==4 &&
    (unsigned long)&((Effect *)0)->pad==6 &&
...
int func_8019320C(void) { assert(0); return 0; }
int func_800CE560(void *p,int stride,int count,int (*callback)()) {
    assert(init++==0 && p==pool && stride==12 && count==9 && callback==func_8019320C);
    return 140;
}
Effect *func_800CE610(void *p) {
    assert(p==pool && allocations++==0);
    D_800F32D0=&replacement;
    D_800E27EC=newCounter;
...
int main(void) {
    ...
    assert(func_801933A0(0)==140 && init==1);
    for(i=0;i<8;++i) for(j=0;j<11;++j) for(k=0;k<11;++k) for(f=0;f<2;++f) {
        int index=(int)(((long long)counters[i]*1024/12)&0xFFF)*2;
        Effect before;
        newCounter=counters[i]; D_800E27EC=0;
        D_800F32D0=&state; state.owner=0;
        memset(D_800966EC,0x55,sizeof(D_800966EC));
        D_800966EC[index]=samples[j]; D_800966EC[index+1]=samples[k];
        memset(&output,0xA5,sizeof(output)); before=output;
        fail=f; allocations=0;
        assert(func_801933A0(1)==0 && allocations==1);
        if(f) assert(memcmp(&output,&before,sizeof(output))==0);
        else {
            assert(output.x==-1 && output.y==-1 && output.z==-32768);
            assert(output.pad==(short)0xA5A5 && output.parameter[3]==0xA5);
            assert(output.parameter[0]==(unsigned char)((long long)samples[j]*40/4096));
            assert(output.parameter[1]==8);
            assert(output.parameter[2]==(unsigned char)((long long)samples[k]*40/4096));
        }
    }
    fail=1;
    for(i=-2;i<=13;++i) {
        allocations=0; D_800E27EC=i;
        assert(func_801933A0(1)==(i>=12 ? 2:0));
        assert(allocations==(i<12));
    }
    allocations=0; D_800E27EC=INT_MAX;
    assert(func_801933A0(1)==2 && allocations==0);
    assert(func_801933A0(2)==0 && func_801933A0(3)==0);
    assert(func_801933A0(INT_MIN)==0 && func_801933A0(INT_MAX)==0 && allocations==0);
    assert(init==1);
    return 0;
}
'''
